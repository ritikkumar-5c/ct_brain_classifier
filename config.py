...
*binary, study-level* task (normal vs abnormal) on DICOM input.
"""
...
class Config:
    ...
    train_csv: str = ""                       # columns: path,label[,slice_size]
    val_csv: str = ""
    test_csv: str = ""                        # "" to skip the held-out test set
    data_root: str = "/path/to/dataset"      # (legacy) root with one folder per study
    labels_csv: str = "/path/to/labels.csv"  # (legacy) columns: study_id,label  (label in {normal,abnormal} or {0,1})
    image_size: int = 224                     # paper: 224x224
    ...
    max_slices_per_study: int = 32            # cap a study's bag size (eval)
    train_slices_per_study: int = 24          # random-sample this many slices per study while training
    ...
    aug_brightness: float = 0.1               # ColorJitter brightness (0 = off)
    aug_contrast: float = 0.1                 # ColorJitter contrast (0 = off)

    # ----- model -----
    # backbone: maxvit (primary, paper) | vit | tnt | convnext (comparison)
    backbone: str = "maxvit"
    timm_name: str = "maxvit_tiny_tf_224.in1k"  # resolved per-backbone in models/build.py
    ...
    batch_size: int = 8                       # NOTE: studies-per-batch (each study = a bag of slices)
    ...
    slice_chunk: int = 0                      # encode slices in chunks of N (0 = all at once, original behavior)
    grad_checkpoint: bool = False             # gradient-checkpoint the backbone (less memory, ~20-30% slower)
